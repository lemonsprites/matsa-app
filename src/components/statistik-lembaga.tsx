...
const StatistikLembaga = () => {
    ...
    return (
        <section className="text-gray-600 body-font bg-gradient-to-bl from-cyan-500 via-teal-500 to-cyan-600">
            <div className="py-24 mx-auto flex flex-wrap matsa-wrapper">
                {/* Animated Image */}
                <motion.div
                    className="lg:w-1/2 sm:w-1/3 w-full rounded-lg shadow-md relative overflow-hidden mt-6 sm:mt-0"
                ...
                {/* Animated Stats */}
                <motion.div
                    className="flex flex-wrap -mx-4 mt-auto mb-auto lg:w-1/2 sm:w-2/3 content-start sm:pl-14"
                    initial={{ opacity: 0, x: 100 }}
                    whileInView={{ opacity: 1, x: 0 }}
                    transition={{ duration: 0.8, ease: "easeOut" }}
                    viewport={{ once: false }}
